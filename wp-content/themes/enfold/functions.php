...
add_shortcode('birth_days', 'show_all_birthdays');
function show_all_birthdays(){
    $user = wp_get_current_user();
    if ( ! in_array( 'administrator', (array) $user->roles ) ) {
        return 'You has no rights!';
    }
    global $wpdb;
    $sql = "SELECT 
	  p.ID as order_ID,
      ...
      pm3.meta_value as 'birth_date',
      pm4.meta_value as 'billing_first_name',
      pm5.meta_value as 'billing_last_name',
      u.*
    FROM ". $wpdb->posts ." as p
    LEFT JOIN " . $wpdb->postmeta . " as pm1 	ON (p.ID = pm1.post_id) AND (pm1.meta_key = '_customer_user')
    LEFT JOIN " . $wpdb->postmeta . " as pm2 	ON (p.ID = pm2.post_id) AND (pm2.meta_key = '_billing_email')
    LEFT JOIN " . $wpdb->postmeta . " as pm3 	ON (p.ID = pm3.post_id) AND (pm3.meta_key = 'My Field')
    LEFT JOIN " . $wpdb->postmeta . " as pm4 	ON (p.ID = pm4.post_id) AND (pm4.meta_key = '_billing_first_name')
    LEFT JOIN " . $wpdb->postmeta . " as pm5 	ON (p.ID = pm5.post_id) AND (pm5.meta_key = '_billing_last_name')
    LEFT JOIN " . $wpdb->users . "  as u 	ON (pm1.meta_value = u.ID)
    ...
    foreach ($results as $result){
        $content .= '<tr>'.
            ...
            '<td>' . $result->billing_first_name . ' ' . $result->billing_last_name  . '</td>'.
    ...
    $tpl = '<table><thead><tr class="tab-headers-birth"><td> OrderID </td><td> First Name + Last Name (from order)</td><td> Registration name </td><td> CustomerID </td><td> Email from Order </td><td> Email from Users </td><td> Birth date </td></tr></thead><tbody>' . $content . '</tbody></table>';
    return $tpl;
}
/*
